...
- New setting `ghostAlign.shortenUrls` (default on) shortens an http(s) URL in a CSV/TSV cell or a Markdown table cell to just its host (e.g. `https://github.com/foo/bar?q=1` displays as `[github.com]`), so one long URL no longer stretches the whole column. Only the scheme/userinfo and path/query/fragment are visually hidden — the host is real, unmodified document text, so Ctrl+F and copy still see the full text, and moving the cursor/selection into a shortened URL temporarily expands it back to full text. Ctrl+click opens exactly that URL via a dedicated link provider — not VS Code's generic link detection, which on an unquoted CSV/TSV cell would otherwise swallow trailing text from the next cell into the same link (an ASCII `,` only ends a link candidate at end of line, not mid-line). Toggling Ghost Align off (`ghostAlign.toggle`) now also clears any shortened-URL decorations, matching every other alignment decoration.
...
- Table-like text inside a Markdown code block no longer receives ghost padding. Fenced code block detection now matches GFM more closely: an opening fence only counts with 0-3 spaces of indentation (4+ is an indented code block), and a closing fence additionally requires the same indentation limit, at least as many fence characters as the opener, and nothing but whitespace after them — so a line like ` ```not-a-closer` inside a fence no longer closes it early. Table-like text inside a 4+-space-indented code block is likewise excluded.
- A CSV/TSV double-quoted field containing a literal newline (RFC 4180) is now parsed correctly across the physical lines it spans: a delimiter inside the still-open quoted field is no longer mistaken for a column separator, and the delimiter that follows the closing quote on a later line is now recognized as one. Single-line quoted fields, `""` escapes, and custom delimiters keep their existing behavior.
- In a file of 10,000+ lines, an operator group longer than the large-file slicing's group-boundary expansion limit (2,000+ lines) is now aligned consistently regardless of scroll position — previously its alignment target only accounted for the rows within the currently expanded slice, so scrolling could shift where the group aligned depending on where its widest row happened to fall relative to the visible range.
...
- A JSDoc `@param`/`@property`/`@arg`/`@argument` line with an optional-parameter name whose default value itself contains `]` (e.g. `[indices=[]]`, `[items=["a","b"]]`) no longer misdetects the name bracket's end at the default value's inner `]`, which was throwing off the description column's alignment with sibling lines.
...
- TS/JS `:` alignment no longer mistakes a `default` member of an `interface`/`type`/object literal (e.g. `interface Foo { default: string; }`) for a `switch` statement's `default:` label, so it now aligns like any other property/member colon. The finder now tracks whether the enclosing `{...}` is actually a `switch` body across lines, instead of relying only on a same-line trailing-comma heuristic.
