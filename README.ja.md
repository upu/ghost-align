...
| `ghostAlign.csv.alignNumbersRight` | boolean | `false` | 列のデータセルが全て単純な数値（先頭の `-` は省略可、数字、小数部も省略可）に見える列だけ、左寄せの代わりに右寄せにします。桁区切りカンマや指数表記は数値として認識しません。1行目（多くはヘッダー）はこの判定からは除外されますが、その列が数値列と判定されれば1行目自体も右寄せされます。数値以外のセルが1つでもある列は従来どおり左寄せのままです。 |
