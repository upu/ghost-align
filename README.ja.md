...
- 既定では `=` を揃えます。言語によって既定のオペレーターが変わり、`json` / `jsonc` / `yaml` / `css` / `scss` / `less` / `graphql` では `:`、`typescript` / `typescriptreact` / `javascript` / `javascriptreact` / `python` では `:` と `=` の両方（Python では `:` が辞書リテラルのキーと型・引数注釈を揃えます。スライス構文、`if x:` のようなブロック開始のコロン、`lambda x: ...` 自身の区切りは対象外です）、`dotenv` / `properties` / `toml` / `ini` / `shellscript` / `makefile` / `go` / `lua` / `c` / `cpp` / `csharp` / `java` / `swift` / `kotlin` / `dart` / `zig` / `terraform` / `proto3` / `elixir` / `perl` / `sql` / `haskell` / `powershell` / `dockerfile` / `scala` / `groovy` では `=` を揃えます。`ruby` / `php` / `rust` では `=` に加えて `=>`（ハッシュロケット・連想配列・match アーム）も揃えます。`r` では `=` に加えて慣用的な代入演算子 `<-` も揃えます。それ以外の言語はグローバルの `ghostAlign.operators`（既定 `=`）にフォールバックします。`ghostAlign.alignUnknownLanguages` を `false` にすると、`operatorsByLanguage` に載っている言語だけを揃えます。
...
| `ghostAlign.csv.alignNumbersRight` | boolean | `false` | 列のデータセルが全て単純な数値（先頭の `-` は省略可、数字、小数部も省略可）に見える列だけ、左寄せの代わりに右寄せにします。桁区切りカンマや指数表記は数値として認識しません。1行目（多くはヘッダー）はこの判定からは除外されますが、その列が数値列と判定されれば1行目自体も右寄せされます。数値以外のセルが1つでもある列は従来どおり左寄せのままです。小数値がある列はセル全体の右寄せでなく小数点の位置で揃えます（小数点のない整数セルは整数部の桁数だけで揃います）。 |
| `ghostAlign.shortenUrls` | boolean | `false` | CSV/TSV のセル（どの列でも）や Markdown テーブルのセルにある http(s) URL を、ホスト部分だけの表示に短縮します（例: `https://github.com/foo/bar?q=1` は `[github.com]` と表示）。これにより長い URL が1つあるだけで列全体が間延びしなくなります。視覚的に隠すのは scheme・userinfo とパス・クエリ・フラグメントのみで、ホスト部分は実際のドキュメントテキストのままなので、Ctrl+クリックで全 URL を開けます。Ctrl+F やコピーも常に全文を対象にし、カーソル・選択が短縮中の URL に入ると一時的に全文表示に戻ります。列の整列は短縮後の幅を基準に計算するため、隠れた URL のぶんで表が間延びすることもありません。 |
...
- **ゴーストパディング付近のマウス選択** — ゴーストパディングは VS Code の Decoration API（`before` アタッチメント）で描画されており、エディタのマウスのヒットテストにはこの描画幅も含まれます。そのため、パディングをまたぐクリック＆ドラッグ選択ではパディングの幅ぶん余分にドラッグする必要があり、パディング直前・直後の文字だけを狙って選択しづらいことがあります。拡張機能 API にはデコレーション内容をヒットテストから除外する手段がなく、拡張機能側では修正できません。キーボードでの選択（`Shift`+矢印キー）には影響しません。正確なマウス選択が必要なときは、`Ghost Align: Toggle` で一時的に OFF にできます。
