...
- 対応する最低 VS Code バージョンを 1.125.0 から 1.136.0 に引き上げ。ビルドに使用する `@types/vscode` のバージョンに合わせた。
...
- 新しい設定 `ghostAlign.shortenUrls`（既定オン）を追加。CSV/TSV セルや Markdown テーブルセル内の http(s) URL をホスト部分だけの表示に短縮する（例: `https://github.com/foo/bar?q=1` は `[github.com]` と表示）。長い URL が1つあるだけで列全体が間延びしなくなる。視覚的に隠すのは scheme・userinfo とパス・クエリ・フラグメントのみで、ホスト部分は実際のドキュメントテキストのままなので、Ctrl+F やコピーも常に全文を対象にする。カーソル・選択が短縮中の URL に入ると一時的に全文表示に戻る。Ctrl+クリックは専用のリンクプロバイダー経由でその URL のみを開く（VS Code 標準のリンク検出だと、半角カンマ区切りの CSV/TSV セルでは行末以外でリンクの区切りとみなされないため、隣のセルの文字列まで巻き込んでしまう）。Ghost Align のトグルを OFF にすると、他の整列デコレーションと同様に短縮表示もクリアされる。
...
- 有効な GFM テーブルでないパイプ・ハイフン混じりのテキストを Markdown テーブルと誤認識してゴーストパディングを付けなくなった。区切り行の各セルは「1個以上のハイフン＋任意の先頭・末尾コロン」を満たす必要があり（`| : | --- |` のような行は拒否）、区切り行とヘッダー行のセル数が一致しない場合もテーブルとして認識しない（GFM の仕様どおり）。`:---` / `---:` / `:---:` のアラインメントマーカーや先頭・末尾パイプの省略を含む有効なテーブルの挙動は従来どおり。
- Markdown のコードブロック内にある表形式テキストへゴーストパディングを付けなくなった。フェンスドコードブロックの判定を GFM 仕様に近づけ、開始フェンスは0〜3スペースの字下げのみ有効（4スペース以上は indented code block）とし、終了フェンスはさらに同じ字下げ制限・開始以上のフェンス文字数・後続が空白のみという条件を満たす場合のみ有効にした。これにより、フェンス内の ` ```not-a-closer` のような行がフェンスを早期に閉じなくなった。4スペース以上字下げされたコードブロック内の表形式テキストも同様に整列対象から除外される。
- 改行を含む CSV/TSV のダブルクォートフィールド（RFC 4180）が、それがまたがる複数の物理行にわたって正しく解析されるようになった。まだ閉じていないクォートフィールド内の区切り文字を列区切りと誤認しなくなり、後続行で閉じクォートの後に続く区切り文字は正しく区切りとして認識されるようになった。単一行のクォートフィールド・`""` エスケープ・カスタム区切り文字の既存挙動は変わらない。
- 1万行以上のファイルで、大ファイル用スライスのグループ境界拡張の上限（2,000行超）を超える operator group が、スクロール位置によらず一貫して揃うようになった。従来はその時点で展開されているスライス内の行だけを基準に整列先を計算していたため、グループ内で最も幅の広い行が可視範囲との位置関係によってスライス内外どちらに入るかでスクロールごとに整列先がずれることがあった。
