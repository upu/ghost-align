...
CSV/TSV の全セル、および Markdown テーブルセル内（テーブル外の本文は対象外）の http(s) URL を、ホスト部分だけの表示に短縮する。`[github.com]` のようにホスト名を実テキストのまま `[` `]` で囲み、scheme・userinfo・パス・クエリ・フラグメントは decoration（`textDecoration` への CSS 注入）で視覚的に隠すだけで、ソースバッファは変更しない。ホストが実テキストのため VS Code 標準のリンク検出・Ctrl+クリックがそのまま働く。カーソル・選択が短縮中の URL に触れると一時的に全文表示へ展開する。列の整列（ゴーストパディング）は短縮後の幅を基準に計算するため、長い URL のせいで列が間延びしない。ブール1つで on/off する（既定オン、長さしきい値なし＝検出したら常に短縮）。
