...
1. **タグ push まで** — `/release x.y.z` スキルが担当します。`CHANGELOG.md` と `CHANGELOG.ja.md` の `[Unreleased]` を `## [x.y.z] - YYYY-MM-DD` に確定し、`package.json` の version を bump、PR → CI green → merge した後、`vx.y.z` タグを作成して push します。
2. **タグ push 後（自動）** — `vx.y.z` タグの push で `.github/workflows/release.yml` が起動し、`npm run package` で `.vsix` を生成 → GitHub Release を作成して `.vsix` を添付 → VS Code Marketplace に公開します。リリースノートは `CHANGELOG.md` の該当版セクションから生成されます。

`main` への通常の push ではリリースは起きません（トリガーは `v*.*.*` タグのみ）。
