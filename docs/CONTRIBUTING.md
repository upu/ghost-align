...
1. **タグ push まで** — `/release x.y.z` スキルが担当します。`CHANGELOG.md` の `[Unreleased]` を `## [x.y.z] - YYYY-MM-DD` に確定し、`package.json` の version を bump、PR → CI green → merge した後、`vx.y.z` タグを作成して push します。
