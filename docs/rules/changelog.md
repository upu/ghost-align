...
README と同じく、**英語版 `CHANGELOG.md` が正**で、日本語版 `CHANGELOG.ja.md` を併記します。

- Marketplace の Changelog タブと GitHub Release のリリースノートは `CHANGELOG.md`（英語）から生成されます。
- エントリは必ず**両方のファイルに同内容で**追記します（英語で書き、日本語訳を ja に）。`## [x.y.z]` / `### Added` などの見出しは両ファイルとも英語のままです。
- `npm run prepare-release` は両ファイルの `[Unreleased]` を検証・確定するため、片方だけ追記された状態はリリース時にエラーで検出されます。
- `.vsix` には英語版のみ含めます（`CHANGELOG.ja.md` は `.vscodeignore` で除外、`README.ja.md` と同じ扱い）。
...
- `upu/agent-marketplace` リポジトリの dev-flow プラグイン — `skills/ship/SKILL.md`（`dev-flow:ship` での Unreleased 追記手順）、`skills/release/SKILL.md`（`dev-flow:release` でのリリース時確定手順。版確定・bump・タグ）
