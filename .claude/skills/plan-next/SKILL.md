...
7. **選ばれた issue を起票する** — 選定した各項目について、背景 / 期待する挙動 / 設計メモ / 受け入れ基準 のセクション構成で本文を書く。`gh issue create --milestone "vX.Y.Z" --label <種別> --label <優先度>` で作成する。
   - 設計に複数の妥当なアプローチがあるときは、計画時点で 1 つに決めない：設計メモに候補案を列挙し（例: 案1/案2/案3 とトレードオフ、推奨があればその旨）、「実装時にどれかを選ぶ（PR で決定を明記）」と書き添える。こうすると `dev-flow:ship` が質問でブロックされずに自律実装でき、決定が PR に監査可能な形で残る（v0.3.0 の #139/#140/#147 でうまくいった方式）。issue の分析は仮説として扱う——検証するのは `dev-flow:ship` のテストファーストのステップ。
...
8. **マイルストーンの description を共有ブリーフとして書く** — 全 issue が揃ったら、後続のセッションやスキル（`dev-flow:ship`、`dev-flow:release`）がユーザーに聞き直さずに計画を引き継げるよう description を更新する：
   - リリースのテーマ・目的を一行で。
...
9. **報告する** — 作成した issue 番号と優先度の一覧、マイルストーンの URL、推奨着手順序を挙げる。各 issue は `dev-flow:ship <n>` で実装すること、引き継ぎブリーフはマイルストーンの description にあること、全 issue がクローズしたらマイルストーン完了で `dev-flow:release` に引き継ぐことも伝える。

## 補足

- これは計画用のスキル：成果物は issue とマイルストーンであって、コードではない。ここで実装を始めない——それは `dev-flow:ship` の仕事。
