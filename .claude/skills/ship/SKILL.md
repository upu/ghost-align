...
5. **Test (gate)** — run `npm run compile`, then `npm test`, then `npm run check:package`. This mirrors what the CI `test` job runs on every PR (`compile` → `test` → `check:package`), so green locally means CI should pass too — skipping `check:package` here lets a packaging regression slip through to a red CI. All must pass. When the acceptance criteria call for it, add or extend tests to prove the behavior. If anything is red, fix it; do not open a PR on a failing build.
