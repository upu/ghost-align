...
- 既定対応言語（`DEFAULT_OPERATORS_BY_LANGUAGE`）の拡充は #238 / #310 で打ち止め。マイナー言語の追加はキリがないため、明示的な要望があった言語のみ個別に検討する（2026-07-08 ユーザー決定）
...
- コメントは最小限。テスト名・関数名・実装から自明な内容や、リファクタでズレる行番号参照は書かない
